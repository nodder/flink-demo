...
import name.cdd.study.flink.demo.onesight.table.function.DiffWithTriggerFunction;
...
//滑动窗口，统计count，只输出变化的内容。增加了过期（超时）删除，以及发送notify的功能。
...
public class OutputNonRepetitive4_hop_trigger
{
    public static void main(String[] args) throws Exception
    {
        ...
        tableEnv.registerFunction(DiffWithTriggerFunction.METHOD_NAME, new DiffWithTriggerFunction(env, kafkaProps, fromInput(pt, "output-topic")));
        ...
        String sql = "select custom_id, "
                     ...
                     + "from KafkaTable GROUP BY custom_id, "
                                      ...
                                      + "HAVING IS_DIFF_WITH_EXPIRE_NOTIFY(custom_id, count(custom_id), INTERVAL '8' SECOND)";
